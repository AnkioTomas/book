...
class MetadataFillTask extends AiTask
{
    ...
    protected function postProcess(array $out): array
    {
        ...
        if (isset($out['rate']) && $out['rate'] !== '') {
            $rate = (float)$out['rate'];
            // 豆瓣/微信读书多为十分制；若模型误把十分制原样写入，先 ÷2 再钳到 0-5
            if ($rate > 5) {
    ...
    public function fill(string $bookName, string $author = '', ?callable $onProgress = null): ?array
    {
        ...
        $prompt = "请为下面这本书检索并挑选最准确的元数据：\n"
            . "书名：{$bookName}\n"
            . ($author !== '' ? "作者：{$author}\n" : '')
            . "优先用 search_douban 检索豆瓣；无结果或信息不全时用 search_weread 检索微信读书补全；必要时用 fetch_url。"
            . "在多个候选里挑选最匹配的一条。"
            ...
            . "rate(本系统五星评分，整数，最低0最高5，满分只有5分；豆瓣与微信读书对外多为十分制，必须先÷2再取整写入，禁止直接写原始分), "
